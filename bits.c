...
Bool bitIsSet(Bits b, int position)
{
	...
	int byte_number = position / 8;
	int byte_offset = position % 8;
	if ((b->bitstring[byte_number] & (1 << byte_offset)))
		return TRUE;
	return FALSE;
}
...
Bool isSubset(Bits b1, Bits b2)
{
	...
	for (i = 0; i < b1 -> nbytes; i ++){
        Byte result = b1->bitstring[i] & b2->bitstring[i];
        if (result != b1->bitstring[i])
            return FALSE;
	}
	return TRUE;
}
...
void setBit(Bits b, int position)
{
	...
	int byte_number = position / 8;
	int byte_offset = position % 8;
	b->bitstring[byte_number] = ((1 << byte_offset) | b->bitstring[byte_number]);
}
...
void unsetBit(Bits b, int position)
{
	...
	int byte_number = position / 8;
	int byte_offset = position % 8;
	b->bitstring[byte_number] &=~ (1 << byte_offset);
}
...
{
	Byte* b_bits = addrInPage(p, pos, b->nbytes);
	memcpy(b->bitstring, b_bits, b->nbytes);
}
...
{
	Byte* b_bits = addrInPage(p,pageNitems(p), b->nbytes);
	memcpy(b_bits, b->bitstring, b->nbytes);
	addOneItem(p);
}
...
void setBitstring(Byte* b, Bits bits){
    int i;
//    printf("\n");
    for (i = 0; i < bits->nbytes; i ++){
        bits->bitstring[i] = b[i];
    }
//    showBits(bits);
}

Byte getBitstring(Bits b, int i) {return b->bitstring[i];}
Byte* getBitstrings(Bits b){
	Byte* bytes = malloc(sizeof(b->nbytes) + 1);
	int i;
	for (i = 0; i < b->nbytes; i ++)
		bytes[i] = b->bitstring[i];
	return bytes;
}

Count getBitsBytes(Bits b) {return b->nbytes;}
