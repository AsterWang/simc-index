...
//    Page p2; PageID pid2;
    ...
    Byte* t2 = getBitstrings(tsig);
    addTupSigToPage(r,p,t2);
    rp->ntsigs ++;
...
//    Page p3; PageID pid3;
    pid = rp->psigNpages - 1;
    Bits newPsig = makePageSig(r, t);
    p = getPage(r->psigf, pid);
    //how many tuple pages
    Count tupleNPages = r->params.npages;
    //how many page signature pages needed
    Count psigNapage = iceil(tupleNPages, rp->psigPP);
    //if current number of page less than the page needed, create a new page.
    printf("psigNapage = %d,rp->psigNpages = %d\n",psigNapage, rp->psigNpages);
    if (psigNapage > rp->psigNpages){
        printf("add page signature page................................\n");
        addPage(r->psigf);
        rp->psigNpages ++;
        pid ++;
        free(p);
    ...
    Byte* t3 = getBitstrings(newPsig);
    mergePageSigToPage(r, p, t3);
    putPage(r->psigf, pid, p);
    printf("after inserting, the page item number is %d\n",pageNitems(p));
